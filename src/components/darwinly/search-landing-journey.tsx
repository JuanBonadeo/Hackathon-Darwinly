...
const sections: JourneySection[] = [
  { id: "search-hook",      label: "Overview",      shortLabel: "Intro"    },
  { id: "search-timeline",  label: "Timeline",      shortLabel: "Timeline" },
  { id: "search-evolution", label: "Evolution Map", shortLabel: "Map"      },
  { id: "search-current",   label: "Today",         shortLabel: "Today"    },
  { id: "search-resources", label: "Resources",     shortLabel: "Refs"     },
]

export function SearchLandingJourney() {
  ...
  useEffect(() => {
    ...
    // Poll for elements that appear progressively (streaming content)
  ...
  // Only show sections already in the DOM
  const visibleSections = sections.filter((s) => observedIds.has(s.id))
      ...
      <div className="fixed inset-x-0 bottom-4 z-30 px-4 md:hidden">
        <div className="mx-auto flex w-full max-w-xs items-center justify-between rounded-full border border-border/80 bg-background/80 px-4 py-2.5 shadow-[0_8px_30px_-22px_rgba(0,0,0,0.55)] backdrop-blur">
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground/80">Deep Dive</p>
            <p className="truncate text-sm font-medium text-foreground">
              {sections[activeIndex]?.shortLabel ?? sections[0].shortLabel}
              <span className="ml-2 text-xs text-muted-foreground">
                {Math.max(activeIndex + 1, 1)}/{visibleSections.length}
              </span>
            </p>
          </div>
